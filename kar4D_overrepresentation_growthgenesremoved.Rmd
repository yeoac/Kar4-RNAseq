---
title: "Overrepresentation Analysis - kar4D with growth genes removed"
...
```{r}
kar4D_up <- read.table(file = "kar4del_DEUpregulated_growthggenesremov.txt", header = TRUE, row.names = 1, sep = "\t")
kar4D_down <- read.table(file = "kar4del_DEDownregulated_growthgenesremov.txt", header = TRUE, row.names = 1, sep = "\t")

kar4D_rownames_up <- rownames(kar4D_up)
kar4D_rownames_down <- rownames(kar4D_down)
```
...
write.table(x = go.annot.df, 
          file = paste0("./GO_annotation/kar4D_up.txt"),
          sep = "\t", 
          col.names = NA)
...
write.table(x = go_overrep.up.df, 
          file = paste0("./GO_overrepresentation/GO_overrep_kar4D_up.txt"),
          sep = "\t", 
          col.names = NA)
```
...
dir.create(path = "./GO_overrepresentation", showWarnings = F)
# Sort DE_results table by Log2FC in decreasing order
DE_resultsDOWN.sorted <- kar4D_down
...
write.table(x = go.annot.down.df, 
          file = paste0("./GO_annotation/GO_annot_kar4_down.txt"),
          sep = "\t", 
          col.names = NA)
...
write.table(x = go_overrep.down.df, 
          file = paste0("./GO_overrepresentation/GO_overrep_kar4D_down.txt"),
          sep = "\t", 
          col.names = NA)
```
...
p1 <- draw_enrichment_barplot(my_enricher_obj = go_overrep.up, my_pathway_counts = 300, file_name_prefix = "./GO_overrepresentation/kar4D_up_growthgenesrem", my_width = 10, my_height = 10, padj_cutoff = 0.05)

p2 <- draw_enrichment_barplot(my_enricher_obj = go_overrep.down, my_pathway_counts = 300, file_name_prefix = "./GO_overrepresentation/kar4D_down_growthgenesrem", my_width = 10, my_height = 10, padj_cutoff = 0.05)
```