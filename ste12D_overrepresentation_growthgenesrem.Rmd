---
title: "Overrepresentation Analysis - ste12D with growth genes removed"
...
```{r}
ste12D_up <- read.table(file = "ste12D_DEUP_growthgenesremov.txt", header = TRUE, row.names = 1, sep = "\t")
ste12D_down <- read.table(file = "ste12D_DEDown_growthgenesremov.txt", header = TRUE, row.names = 1, sep = "\t")

ste12D_rownames_up <- rownames(ste12D_up)
ste12D_rownames_down <- rownames(ste12D_down)
```
...
write.table(x = go.annot.df, 
          file = paste0("./GO_annotation/ste12D_up_growthgenesrem.txt"),
          sep = "\t", 
          col.names = NA)
...
go_overrep.up <- enrichGO(keyType = "ORF",
                gene = rownames(subset(DE_results.sorted)),
...
write.table(x = go_overrep.up.df, 
          file = paste0("./GO_overrepresentation/GO_overrep_ste12D_up_growthgenesrem.txt"),
          sep = "\t", 
          col.names = NA)
```
...
dir.create(path = "./GO_overrepresentation", showWarnings = F)
# Sort DE_results table by Log2FC in decreasing order
DE_resultsDOWN.sorted <- ste12D_down
...
write.table(x = go.annot.down.df, 
          file = paste0("./GO_annotation/GO_annot_ste12_down_growthgenesrem.txt"),
          sep = "\t", 
          col.names = NA)
...
write.table(x = go_overrep.down.df, 
          file = paste0("./GO_overrepresentation/GO_overrep_ste12D_dowgrowthgenesrem.txt"),
          sep = "\t", 
          col.names = NA)
```
...
p1 <- draw_enrichment_barplot(my_enricher_obj = go_overrep.up, my_pathway_counts = 300, file_name_prefix = "./GO_overrepresentation/ste12D_up_growthgenesrem", my_width = 10, my_height = 10, padj_cutoff = 0.05)

p2 <- draw_enrichment_barplot(my_enricher_obj = go_overrep.down, my_pathway_counts = 300, file_name_prefix = "./GO_overrepresentation/ste12D_down_growthgenesrem", my_width = 10, my_height = 10, padj_cutoff = 0.05)
```